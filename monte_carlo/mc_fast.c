...
double calculer_energie_potentielle() {
    ...
    #pragma omp parallel for reduction(+:energie)
    for (int i = 0; i < N; i++) {
        for (int j = i + 1; j < N; j++) {
            ...
            double r_over_sigma_sixth = r_over_sigma_sqr * r_over_sigma_sqr * r_over_sigma_sixth;
...
int main() {
    ...
    // Write trajectory data to CSV files
    for (int step = 0; step < STEPS; step++) {
        for (int i = 0; i < N; i++) {
            fprintf(fichier_x, "%lf,", trajectoires_x[step][i]);
            fprintf(fichier_y, "%lf,", trajectoires_y[step][i]);
